...
(async () => {
  ...
  await page.evaluate(() => select({ type: "area", id: "MALL-GREEN" }, { fly: false }));
  ...
  const deep = await page.evaluate(() => poisUnder("ARC-MARUGAMEMACHI", true).length);
  check("descendant points roll up to the arcade", deep >= 1, `${deep} point(s)`);
  ...
  const before = await page.evaluate(() => pois.length);
  await page.evaluate(() => {
    const a = areaById.get("ARC-TAMACHI");
    const [lo, la] = a.label_at;
